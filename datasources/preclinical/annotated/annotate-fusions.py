...
def annotate_data(df, db, genes, consider_partner=False):
    ...
    df = df.merge(
        db.loc[:, match_columns + ["evidence_map", "merged"]]
    ...
    if consider_partner:
        ...
        df = df.merge(
            db[~db["alteration_type"].eq("") & ~db["partner"].eq("")]
            .loc[:, match_columns + ["evidence_map", "merged"]]
            .sort_values("evidence_map", ascending=False)
            .drop_duplicates(match_columns, keep="first"),
            on=match_columns,
...
def annotate_both_genes(df_gene1, df_gene2, db_gene1, db_gene2, almanac_genes, df, dbs):
    ...
    df = df.sort_values(["evidence", "feature_match"], ascending=False).drop_duplicates(
        ["feature", "partner", "model_id"], keep="first"
    )
...
def annotate_gene_1(df_gene1, db_gene1, db_gene2, almanac_genes, dbs):
    ...
    group1 = group1.sort_values(
        ["evidence", "feature_match"], ascending=False
...
def load_db(almanac_json, table_label):
    ...
    db.sort_values(
        ["evidence_map", "feature_display"], ascending=[False, True], inplace=True
    )
    db["merged"] = 1
    return db
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Annotate somatic variants with MOAlmanac"
    )
    ...
    db_gene1 = (
        db.copy(deep=True)
        .rename(columns={"gene1": "feature", "gene2": "partner"})
        .fillna("")
    )
    db_gene2 = (
        db.copy(deep=True)
        .rename(columns={"gene1": "partner", "gene2": "feature"})
        .fillna("")
    )

    both_genes = annotate_both_genes(
        df_gene1, df_gene2, db_gene1, db_gene2, almanac_genes, dataframe, dbs
    )
