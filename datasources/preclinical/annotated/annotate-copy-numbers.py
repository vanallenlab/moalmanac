...
def load_db(almanac_json, table_label):
    ...
    db_columns = ["feature_display", "gene", "direction", "predictive_implication"]
    db = db.loc[:, db_columns].drop_duplicates()
    db.rename(
        columns={
            "gene": "feature",
            "direction": "alteration_type",
...
def load_df(handle, feature_type):
    df = pd.read_csv(
        handle, sep="\t", usecols=["model_id", "feature", "alteration_type"]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", "-i", type=str, help="input copy number alterations")
