...
def annotate_match_4(df):
    ...
    df = df.merge(
        db[~db["alteration_type"].eq("") & ~db["alteration"].eq("")]
        .loc[:, match_columns + ["evidence_map", "merged"]]
        .sort_values("evidence_map", ascending=False)
        .drop_duplicates(match_columns, keep="first"),
        on=match_columns,
...
def load_db(almanac_json, table_label):
    ...
    db["variant_annotation"].replace(
        {"Oncogenic Mutations": "", "Activating mutation": ""}, inplace=True
    )
...
def load_df(handle, feature_type):
    df = pd.read_csv(
        handle,
        sep="\t",
        usecols=["model_id", "feature", "alteration_type", "alteration"],
    )
