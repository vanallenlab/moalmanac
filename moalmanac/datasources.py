...
class CancerHotspots3D:
    ...
    column_map = {"Gene": gene, "alteration": alt}
...
class Cosmic:
    ...
    column_map = {"Gene name": gene, "Mutation AA": alt}
...
class ExAC:
    ...
    column_map = {"CHROM": chr, "POS": start, "REF": ref, "ALT": alt, "AF": af}
...
class Oncotree:
    ...
    column_map = {"name": ontology, "code": code}
...
class Preclinical:
    ...
    def generate_sample_list(dataframe, use_column, sample_column):
        return (
            dataframe[dataframe[use_column].astype(bool).astype(int).eq(1)][
                sample_column
            ]
            .sort_values()
            .tolist()
