...
class Matchmaker:
    ...
    @classmethod
    def concat_case_comparisons(
        cls, somatic, dbs, variant_string, copy_number_string, rearrangement_string
    ):
        somatic[cls.model_id] = cls.case_profile
        case_variants = cls.subset_dataframe_eq(
            somatic, cls.feature_type, variant_string
        )
        case_cns = cls.subset_dataframe_eq(
            somatic, cls.feature_type, copy_number_string
        )
        case_fusions = cls.subset_dataframe_eq(
            somatic, cls.feature_type, rearrangement_string
        )
        ...
        fusions = cls.concat_dataframes(
            case_fusions, comparison_fusions, fusion_columns
        )
    ...
    def compare(cls, dbs, dbs_preclinical, somatic, case_sample_id, config):
        ...
        calculated = SNFTypesCGCwithEvidence.calculate(
            annotated, samples_to_use, cgc, almanac
        )
    ...
    def format_fusions(cls, dataframe):
        df = (
            ...
            .rename(columns={0: "feature", 1: "partner"})
        )
...
class Models:
    ...
    def calculate_distance(dataframe, metric):
        ...
        return pd.DataFrame(
            distance, columns=dataframe.index.tolist(), index=dataframe.index.tolist()
        )
    ...
    def stack_distances(cls, dataframe, label):
        ...
        stacked.rename(
            columns={"level_0": cls.case, "level_1": cls.comparison, 0: label},
            inplace=True,
        )
...
class Almanac(Models):
    ...
    def import_dbs(cls, inputs):
        ...
        for table in [cls.variant, cls.copy_number, cls.rearrangement]:
            tables[table] = AnnotatorAlmanac.subset_records(
                db["content"], AnnotatorAlmanac.feature_type, table
            )
    ...
    def generate_features(cls, dbs):
        missense = cls.generate_features_missense(
            dbs[cls.variant], cls.gene, cls.variant_annotation, cls.protein_change
        )
        truncating = cls.generate_features_truncating_aggregated(
            dbs[cls.variant], cls.gene, cls.variant_annotation
        )
        nonspecific_variant = cls.generate_features_nonspecific_variant(
            dbs[cls.variant]
        )
        copy_number = cls.generate_features_copy_number(
            dbs[cls.copy_number], cls.gene, cls.direction
        )
    ...
    def generate_features_copy_number(cls, db, column_gene, column_type):
        series = cls.aggregate_columns_by_row(
            db.loc[:, [column_gene, column_type]], " "
        )
        return cls.series_to_list(series, drop_duplicates=True)
    ...
    def generate_features_missense(cls, db, column_gene, column_type, column_protein):
        ...
        series = cls.aggregate_columns_by_row(
            db.loc[idx, [column_gene, column_protein]], " "
        )
        return cls.series_to_list(series, drop_duplicates=True)

    @classmethod
    def generate_features_missense_aggregated(
        cls, db, column_gene, column_type, column_protein
    ):
        idx = db[column_type].eq(cls.missense) & ~db[column_protein].fillna("").eq("")
    ...
    def generate_features_truncating(cls, db, column_gene, column_type):
        ...
        series = cls.aggregate_columns_by_row(
            db.loc[idx, [column_gene, column_type]], " "
        )
        return cls.series_to_list(series, drop_duplicates=True)
    ...
    def generate_gene_features_dtype(cls, dbs):
        ...
        copy_number = (
            dbs[cls.copy_number][cls.gene].drop_duplicates() + f" {cls.copy_number}"
        )
        rearrangements = (
            pd.concat(
                [dbs[cls.rearrangement][cls.gene1], dbs[cls.rearrangement][cls.gene2]]
            )
            .drop_duplicates()
            .dropna()
            .sort_values()
...
class AlmanacEvidence(Almanac):
    label = "FDA features"
    description = (
        "We sort by agreement based measure (jaccard) by considering somatic variant, copy number, and "
    ...
    def calculate(cls, input_dtypes, samples_list):
        ...
        boolean_dataframe = AlmanacFeatures.create_boolean_table(
            input_dtypes, samples_list, almanac_subset
        )
        distance_dataframe = AlmanacFeatures.calculate_distance(
            boolean_dataframe, cls.jaccard.pairwise
        )
    ...
    def subset_almanac_by_evidence(cls, db, evidence_tier):
        ...
        for table in [cls.variant, cls.copy_number, cls.rearrangement]:
            ...
            db_table = AnnotatorAlmanac.subset_records(
                db_table, cls.predictive_implication, evidence_tier
            )
...
class AlmanacFeatures(Almanac):
    ...
    description = (
        "We sort by agreement based measure (jaccard) by considering all somatic variant, copy number, and "
    ...
    def calculate(cls, input_dtypes, samples_list):
        almanac = cls.import_dbs(input_dtypes)
        boolean_dataframe = cls.create_boolean_table(
            input_dtypes, samples_list, almanac
        )
        distance_dataframe = cls.calculate_distance(
            boolean_dataframe, cls.jaccard.pairwise
        )
        stacked_dataframe = cls.stack_distances(distance_dataframe, cls.label)
        return stacked_dataframe
    ...
    def create_boolean_table(cls, inputs, samples, almanac):
        ...
        columns = [
            cls.variant_missense,
            cls.variant_truncating,
            "variant_nonspecific",
            cls.copy_number,
            cls.fusions,
            cls.fusions_gene1,
        ...
        missense[cls.string] = cls.aggregate_columns_by_row(
            missense.loc[:, [cls.feature, cls.alteration]], " "
        )
        ...
        fusions[cls.string] = cls.aggregate_columns_by_row(
            fusions.loc[:, [cls.feature, cls.partner]], "--"
        )
        ...
        df = cls.reset_multi_indexed_dataframe(
            df.fillna(0), {"level_0": cls.string, "level_1": cls.model_id}
        )
    ...
    def subset_fusions(cls, df):
        ...
        return cls.sort_fusions(
            dataframe.reset_index(drop=True), cls.feature, cls.partner
        )
    ...
    def subset_nonspecific_variant(cls, df, db):
        ...
        return df[
            df[cls.feature_match_2].eq(1) & df[cls.feature].isin(relevant_genes)
    ...
    def subset_truncating(cls, df):
        return df[
            df[cls.feature_match_3].eq(1)
            & df[cls.alteration_type].isin(cls.truncating_types)
        ].reset_index(drop=True)
...
class CGC(Models):
    ...
    input_label = "cgc"
    description = (
        "We sort by agreement based measure (jaccard) by considering any variant in a "
    ...
    def create_bool(cls, dataframe, value_column, dtype_column):
        df = (
            dataframe[dataframe[value_column].astype(int).eq(1)]
            .loc[:, [cls.feature, cls.model_id]]
            .drop_duplicates()
        )
        df[dtype_column] = 1
    ...
    def create_boolean_table(cls, inputs, samples):
        ...
        index = pd.MultiIndex.from_product([features, samples])
        df = pd.DataFrame(
            columns=[cls.variants, cls.cnas, cls.fusions_gene1, cls.fusions_gene2],
            index=index,
        )
        ...
        df[cls.fusions_gene1] = cls.create_bool(
            fusions_gene1, cls.cgc_bin, cls.fusions_gene1
        )
        df[cls.fusions_gene2] = cls.create_bool(
            fusions_gene2, cls.cgc_bin, cls.fusions_gene2
        )
        df = cls.reset_multi_indexed_dataframe(
            df.fillna(0), {"level_0": cls.feature, "level_1": cls.model_id}
        )
    ...
    @classmethod
    def create_boolean_table_single_feature_type(
        cls, inputs, samples, feature_type, cgc
    ):
        features = cls.create_features_list(cgc)
...
class SNFTypesCGCwithEvidence(Models):
    ...
    def calculate(cls, inputs, samples, cgc, almanac, seed=42):
        boolean_dataframe_variants = CGC.create_boolean_table_single_feature_type(
            inputs, samples, cls.variants, cgc
        )
        boolean_dataframe_copy_numbers = CGC.create_boolean_table_single_feature_type(
            inputs, samples, cls.cnas, cgc
        )
        boolean_dataframe_fusions = CGC.create_boolean_table_single_feature_type(
            inputs, samples, cls.fusions, cgc
        )

        almanac_content = almanac["content"]
        almanac_subset = AlmanacEvidence.subset_almanac_by_evidence(
            almanac_content, "FDA-Approved"
        )
        boolean_dataframe_1 = AlmanacFeatures.create_boolean_table(
            inputs, samples, almanac_subset
        )
        ...
        affinity_networks = SNF.make_affinity(
            data, metric="jaccard", normalize=False, K=20, mu=0.5
        )
        fused_network = SNF.snf(affinity_networks, K=20)
        fused_dataframe = pd.DataFrame(fused_network, index=samples, columns=samples)
        distance_dataframe = pd.DataFrame(1, index=samples, columns=samples).subtract(
            fused_dataframe
        )
        stacked_dataframe = cls.stack_distances(distance_dataframe, cls.label)
