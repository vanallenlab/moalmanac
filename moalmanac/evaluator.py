...
class Evaluator:
    ...
    sensitive_implication_map = COLNAMES[datasources_section][
        "sensitive_implication_map"
    ]
    resistance_implication_map = COLNAMES[datasources_section][
        "resistance_implication_map"
    ]
    prognostic_implication_map = COLNAMES[datasources_section][
        "prognostic_implication_map"
    ]
    ...
    sensitive_therapy_strategy = COLNAMES[datasources_section][
    ...
    resistance_therapy_strategy = COLNAMES[datasources_section][
        "resistance_therapy_strategy"
    ]
    ...
    def remove_low_allele_fraction_variants(cls, df, minimum_allele_fraction=0.05):
        ...
        idx_mut = df[
            df[cls.feature_type].isin(["Somatic Variant", "Germline Variant"])
        ].index
        idx_low_quality = df[
            df[cls.tumor_f].astype(float).lt(float(minimum_allele_fraction))
        ].index
    ...
    def remove_low_coverage_variants(cls, df, minimum_coverage=15):
        ...
        idx_low_quality = df[
            df[cls.coverage].astype(float).le(float(minimum_coverage))
        ].index
        idx_low_quality_muts = idx_mut.intersection(idx_low_quality)
        idx = df.index.difference(idx_low_quality_muts)
        return df.loc[idx, :]
...
class Actionable:
    ...
    def display_signature(cls, df, idx):
        ...
        signature = df.loc[
            idx, Evaluator.feature
        ]  # .str.replace(before_string, after_string)
        contribution = (
            df.loc[idx, Evaluator.alt]
            .astype(float)
            .multiply(100)
            .round(0)
            .astype(int)
    ...
    @classmethod
    def evaluate(
        cls, somatic, germline, ms_variants, ms_status, burden, signatures, wgd, config
    ):
        ...
        df = features.Features.concat_list_of_dataframes(
    ...
    def format_feature_display(cls, df, config):
        ...
        df.loc[idx_somatic, display_column] = cls.display_variant(
            df=df, idx=idx_somatic
        )
        df.loc[idx_germline, display_column] = cls.display_variant(
            df=df, idx=idx_germline
        )
        ...
        df.loc[idx_msi, display_column] = cls.display_microsatellite_stability(
            df=df, idx=idx_msi
        )
        df.loc[idx_msi_variants, display_column] = cls.display_microsatellite_variants(
            df=df, idx=idx_msi_variants
        )
        df.loc[idx_burden, display_column] = cls.display_burden(df=df, idx=idx_burden)
        df.loc[idx_signature, display_column] = cls.display_signature(
            df=df, idx=idx_signature
        )
    ...
    def format_mutations(cls, df):
        for column in [
            Evaluator.alt_type,
            Evaluator.sensitive_alt_type,
            Evaluator.resistance_alt_type,
            Evaluator.prognostic_alt_type,
        ]:
            df.loc[df.index, column] = cls.format_variant_classification(
                df.loc[df.index, column].fillna("")
            )
        return df
    ...
    def summarize_ms_variants(cls, df, config):
        ...
        if not df.empty:
            ...
            msi_summary.loc[0, Evaluator.feature_type] = config["feature_types"][
                "microsatellite"
            ]
            ...
            msi_summary.loc[0, Evaluator.feature_display] = (
                feature + ": " + feature_displays_list
            )
        return msi_summary
...
class Integrative:
    ...
    def evaluate(cls, somatic, germline, dbs, config):
        ...
        somatic_mutations = cls.extract_feature_type(
            somatic, config["feature_types"]["mut"]
        )
        somatic_copynumbers = cls.extract_feature_type(
            somatic, config["feature_types"]["cna"]
        )
        somatic_fusions = cls.extract_feature_type(
            somatic, config["feature_types"]["fusion"]
        )

        for gene in df.index:
            gene_muts = somatic_mutations[
                somatic_mutations[cls.feature].astype(str) == gene
            ]
            gene_cnas = somatic_copynumbers[
                somatic_copynumbers[cls.feature].astype(str) == gene
            ]
            gene_fusion = somatic_fusions[
                somatic_fusions[cls.feature].astype(str) == gene
            ]
            ...
            df.loc[gene, cls.somatic] = cls.join_alteration_types(
                gene_muts, [cls.alt_type, cls.alt]
            )
            df.loc[gene, cls.germline] = cls.join_alteration_types(
                gene_germline, [cls.alt_type, cls.alt]
            )
...
class Microsatellite(object):
    ...
    def evaluate_status(cls, df, variants):
        columns = [
            Evaluator.almanac_bin,
            Evaluator.sensitive_bin,
            Evaluator.resistance_bin,
            Evaluator.prognostic_bin,
        ]
        if variants.empty:
            for bin_column in columns:
                df[bin_column] = Evaluator.remap_almanac_bins(
                    series=df[bin_column], old_value=3, new_value=2
                )
    ...
    def evaluate_variants(cls, somatic, germline):
        ...
        return features.Features.concat_list_of_dataframes(
            list_of_dataframes=msi_variants
        )
...
class Strategies:
    ...
    @classmethod
    def iterate_over_strategies(
        cls, source_df, strategy_column, therapy_column, target_df, target_row
    ):
    ...
    def report_therapy_strategies(cls, dataframe):
        sensitive_strategies = cls.series_to_list(
            dataframe.loc[:, cls.sensitive_therapy_strategy].dropna()
        )
        resistance_strategies = cls.series_to_list(
            dataframe.loc[:, cls.resistance_therapy_strategy].dropna()
        )
        union_strategies = cls.get_union_strategies(
            sensitive_strategies, resistance_strategies
        )

        df = pd.DataFrame(
            "", index=[cls.sensitivity, cls.resistance], columns=union_strategies
        )
        df = cls.iterate_over_strategies(
            dataframe,
            cls.sensitive_therapy_strategy,
            cls.sensitive_therapy_name,
            df,
            cls.sensitivity,
        )
        df = cls.iterate_over_strategies(
            dataframe,
            cls.resistance_therapy_strategy,
            cls.resistance_therapy_name,
            df,
            cls.resistance,
        )
        return df
