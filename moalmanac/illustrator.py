...
class Illustrator(object):
    ...
    for color in tableau10.keys():
        ...
        tableau10[color] = (r / 255.0, g / 255.0, b / 255.0)
...
class PreclinicalEfficacy(Illustrator):
    ...
    def plot_boxplots(cls, dictionary, drug, features):
        def draw_boxplot(data, offset, width, axis, edge_color, alpha):
            ...
            bp = axis.boxplot(
                data, positions=pos, widths=width, manage_ticks=False, showfliers=False
            )
            for element in ["boxes", "whiskers", "caps"]:
                plt.setp(bp[element], color=Illustrator.tableau10["grey"], linewidth=2)
            for element in ["medians"]:
                plt.setp(
                    bp[element], color=Illustrator.tableau10["orange"], linewidth=2
                )
            return bp

        def create_legend(
            color1, color2, label1, label2, font_size, location, frame_on
        ):
            item1 = Line2D(
                [0],
                [0],
        ...
        def create_title(string):
            return (
                "Sensitivity of wild-type and mutant cell lines to\n{} (GDSC)".format(
                    string
                )
            )
        ...
        bp_mut = draw_boxplot(
            mut_values, -0.15, 0.25, ax, Illustrator.tableau10["blue"], 0.50
        )
        bp_wt = draw_boxplot(
            wt_values, +0.15, 0.25, ax, Illustrator.tableau10["grey"], 0.40
        )
        ...
        create_legend(
            Illustrator.tableau10["blue"],
            Illustrator.tableau10["grey"],
            "Mutant",
            "Wild type",
            16,
            9,
...
class ValidationOverlap(Illustrator):
    ...
    columns = {
        "str": [gene, feature_type, alt],
        "float": [
            tumor_f,
            coverage,
            validation_coverage,
            validation_tumor_f,
            validation_detection_power,
        ],
    }
    ...
    @classmethod
    def plot_overlap_af(
        cls, data, title="", minimum_power=0.95, minimum_allele_fraction=0.05
    ):
        ...
        powered = data[
            data[cls.validation_detection_power].astype(float) >= minimum_power
        ]
        lowpower = data[
            data[cls.validation_detection_power].astype(float) < minimum_power
        ]
        ...
        for idx in data.index:
            ...
            if (primary_tumor_f >= minimum_allele_fraction) & (
                validation_tumor_f >= minimum_allele_fraction
            ):
                ax.annotate(labels[idx], (primary_tumor_f, validation_tumor_f))

        plt.scatter(
            powered[cls.tumor_f].tolist(),
            powered[cls.validation_tumor_f].tolist(),
            color=Illustrator.tableau10["blue"],
            label="".join(
                ["Detection power in validation sequencing >= {}".format(minimum_power)]
            ),
        )
        plt.scatter(
            lowpower[cls.tumor_f].tolist(),
            lowpower[cls.validation_tumor_f].tolist(),
    ...
    def generate_dna_rna_plot(cls, df, patient_id, folder, config):
        ...
        minimum_allele_fraction = float(
            config["validation_sequencing"]["min_af_for_annotation"]
        )
