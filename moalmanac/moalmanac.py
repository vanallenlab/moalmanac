...
class Load:
    ...
    def aneuploidy(boolean, config):
        if boolean:
            logger.Messages.general(message="Whole-genome doubling specified")
        else:
            logger.Messages.general(
                message="Whole-genome doubling not specified, assuming no whole-genome doubling.",
            )
    ...
    def copy_number(path_called, path_seg, config):
        if path_called != "":
            ...
            logger.Messages.general(
                message="No path provided for either called or segmented copy number",
            )
        ...
        if path_called or path_seg:
            logger.Messages.dataframe_size(
                label="...Copy number alterations, passed",
                dataframe=accept,
            )
            logger.Messages.dataframe_size(
                label="...Copy number alterations, rejected",
                dataframe=reject,
            )
        else:
    ...
    def cosmic_mutational_signatures(path, config):
        if path:
            ...
            logger.Messages.dataframe_size(
                label="...imported dataframe shape:",
    ...
    def germline(path, config):
        if path:
            ...
            logger.Messages.dataframe_size(
                label="...germline variants, passed",
                dataframe=accept,
            )
            logger.Messages.dataframe_size(
    ...
    def microsatellite_stability(boolean, config):
        if boolean:
            ...
            logger.Messages.general(message="Microsatellite status not specified.")
    ...
    def somatic_indels(path, config):
        if path:
            ...
            logger.Messages.dataframe_size(
                label="...somatic indel variants, passed",
                dataframe=accept,
            )
            logger.Messages.dataframe_size(
                label="...somatic indel variants, rejected",
                dataframe=reject,
            )
        else:
            logger.Messages.general(
    ...
    def somatic_summary(passed, rejected):
        logger.Messages.dataframe_size(
            label="Overall somatic events, passed",
            dataframe=passed,
        )
        logger.Messages.dataframe_size(
    ...
    def somatic_variants(path, config):
        if path:
            ...
            logger.Messages.dataframe_size(
                label="...Somatic variants, passed",
                dataframe=accept,
            )
            logger.Messages.dataframe_size(
                label="...Somatic variants, rejected",
                dataframe=reject,
            )
        else:
            logger.Messages.general(
    ...
    def validation_sequencing_variants(path, config):
        if path:
            ...
            logger.Messages.dataframe_size(
                label="...validation sequencing variants, passed",
                dataframe=accept,
            )
            logger.Messages.dataframe_size(
                label="...validation sequencing variants, rejected",
                dataframe=reject,
            )
        else:
            logger.Messages.general(
...
class Process:
    ...
    def germline_variants(df, dbs, ontology, config):
        if not df.empty:
            ...
            logger.Messages.general(
                message="Annotation of germline variants complete",
                add_line_break=True,
            )
            ...
            logger.Messages.general(
                message="Evaluation of germline variants complete",
                add_line_break=True,
            )
        else:
            logger.Messages.general(
    ...
    def microsatellite_status(
        ...
        variants = evaluator.Microsatellite.evaluate_variants(
            somatic=somatic_variants,
            germline=germline_variants,
        )
    ...
    def somatic_variants(df, dbs, ontology, config, path_validation):
        if not df.empty:
            ...
            logger.Messages.general(
                message="Annotation of somatic variants complete",
                add_line_break=True,
            )
            ...
            evaluated = evaluator.Evaluator.evaluate_somatic(annotated)
            logger.Messages.general(
                message="Evaluation of somatic variants complete",
                add_line_break=True,
            )
...
def start_logging(patient, inputs, output_folder, config, dbs, dbs_preclinical):
    ...
    if dbs_preclinical:
        logger.Messages.inputs(
            label="Datasources for preclinical modules",
            dictionary=dbs_preclinical,
        )
    else:
        logger.Messages.general(
    ...
    for section in config.sections():
        ...
        logger.Messages.inputs(
            label=f"Config section: {section}",
    ...
    logger.Messages.general(
        message="Logging for inputs provided complete.",
        add_line_break=True,
    )
...
def main(patient, inputs, output_folder, config, dbs, dbs_preclinical=None):
    ...
    mapped_ontology = ontologymapper.OntologyMapper.map(
        dbs,
        metadata_dictionary[tumor_type],
    )
    ...
    df_snv, df_snv_reject = Load.somatic_variants(
        path=inputs[snv_handle],
        config=config,
    )
    df_indel, df_indel_reject = Load.somatic_indels(
        path=inputs[indel_handle],
        config=config,
    )
    df_cnv, df_cnv_reject = Load.copy_number(
        path_called=inputs[called_cn_handle],
        path_seg=inputs[cnv_handle],
        config=config,
    )
    df_fusion, df_fusion_reject = Load.fusions(
        path=inputs[fusion_handle],
        config=config,
    )
    ...
    tumor_whole_genome_doubling = Load.aneuploidy(
        boolean=metadata_dictionary[wgd],
        config=config,
    )
    tumor_microsatellite_status = Load.microsatellite_stability(
        boolean=metadata_dictionary[ms_status],
        config=config,
    )
    ...
    if dbs_preclinical is not None:
        if preclinical_efficacy_on or model_similarity_on:
            ...
            if preclinical_efficacy_on:
                ...
                logger.Messages.general(
                    message="Annotating clinically relevant findings with preclinical efficacy calculations...",
                )
                ...
                logger.Messages.general(message="...annotation complete.")
                logger.Messages.general(
        ...
        logger.Messages.general(
            message="No preclinical databases provided, skipping",
            add_line_break=True,
        )

    logger.Messages.header(label="Writing outputs")
    output_actionable = writer.Actionable.write(
        actionable,
        string_id,
        output_folder,
    )
    ...
    output_msi = writer.MSI.write(
        evaluated_microsatellite_status_variants,
        string_id,
        output_folder,
    )
    output_tmb = writer.MutationalBurden.write(
        evaluated_tumor_mutational_burden,
        string_id,
        output_folder,
    )
    ...
    if function_toggle.getboolean("generate_figures"):
        ...
        logger.Messages.general(
            message="Generating figures disabled, skipping",
            add_line_break=True,
        )
    ...
    if function_toggle.getboolean("generate_actionability_report"):
        ...
        logger.Messages.general(
            message="Generating report disabled, skipping",
            add_line_break=True,
        )
