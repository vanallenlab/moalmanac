...
class Load:
    ...
    def aneuploidy(boolean, config):
        if boolean:
            ...
            logger.Messages.general(
                message=f"Whole-genome doubling not specified, assuming no whole-genome doubling."
            )
    ...
    def copy_number(path_called, path_seg, config):
        if path_called != "":
            ...
            logger.Messages.general(
                message=f"No path provided for either called or segmented copy number"
            )
        accept, reject = features.CopyNumber.import_feature(
            path_called, path_seg, config
        )
        if path_called or path_seg:
            logger.Messages.dataframe_size(
                label="...Copy number alterations, passed", dataframe=accept
            )
            logger.Messages.dataframe_size(
                label="...Copy number alterations, rejected", dataframe=reject
            )
        else:
    ...
    def cosmic_mutational_signatures(path, config):
        if path:
            ...
            signatures = features.CosmicSignatures.import_feature(
                path=path, config=config
            )
            logger.Messages.dataframe_size(
                label="...imported dataframe shape:", dataframe=signatures
            )
        else:
            logger.Messages.general(
                message="No input file for COSMIC mutational signatures provided"
            )
            logger.Messages.general(message="...no input file provided to load")
            signatures = features.CosmicSignatures.import_feature(
                path=path, config=config
    ...
    def fusions(path, config):
        if path:
            ...
            logger.Messages.dataframe_size(
                label="...fusions, rejected", dataframe=reject
            )
        else:
    ...
    def germline(path, config):
        if path:
            ...
            accept, reject = features.MAFGermline.import_feature(
                handle=path, config=config
            )
            logger.Messages.dataframe_size(
                label="...germline variants, passed", dataframe=accept
            )
            logger.Messages.dataframe_size(
                label="...germline variants, rejected",
                dataframe=reject,
                add_line_break=True,
            )
        else:
            logger.Messages.general(
                message="No input file for germline variants provided"
            )
            logger.Messages.general(
                message="...no input file provided to load", add_line_break=True
            )
            accept, reject = features.MAFGermline.import_feature(
                handle=path, config=config
            )
    ...
    def microsatellite_stability(boolean, config):
        if boolean:
            logger.Messages.general(
                message=f"Microsatellite status specified as: {boolean}"
            )
        else:
    ...
    def somatic_indels(path, config):
        if path:
            ...
            accept, reject = features.MAFSomatic.import_feature(
                handle=path, config=config
            )
            logger.Messages.dataframe_size(
                label="...somatic indel variants, passed", dataframe=accept
            )
            logger.Messages.dataframe_size(
                label="...somatic indel variants, rejected", dataframe=reject
            )
        else:
            logger.Messages.general(
                message="No input file for somatic indel variants provided"
            )
            logger.Messages.general(message="...no input file provided to load")
            accept, reject = features.MAFSomatic.import_feature(
                handle=path, config=config
    ...
    def somatic_summary(passed, rejected):
        logger.Messages.dataframe_size(
            label="Overall somatic events, passed", dataframe=passed
        )
        logger.Messages.dataframe_size(
            label="Overall somatic events, rejected",
            dataframe=rejected,
            add_line_break=True,
        )
    ...
    def somatic_variants(path, config):
        if path:
            ...
            logger.Messages.dataframe_size(
                label="...Somatic variants, passed", dataframe=accept
            )
            logger.Messages.dataframe_size(
                label="...Somatic variants, rejected", dataframe=reject
            )
        else:
            logger.Messages.general(
                message="No input file for somatic variants provided"
            )
            logger.Messages.general(message="...no input file provided to load")
            accept, reject = features.MAFSomatic.import_feature(
                handle=path, config=config
    ...
    def tumor_mutational_burden(path, patient_dictionary, variants, dbs, config):
        ...
        if path:
            ...
            logger.Messages.general(
                message=f"Somatic coding tumor mutational burden calculated"
            )
    ...
    def validation_sequencing_variants(path, config):
        if path:
            logger.Messages.general(
                message=f"Validation sequencing variants from {path}"
            )
            accept, reject = features.MAFValidation.import_feature(
                handle=path, config=config
            )
            logger.Messages.dataframe_size(
                label="...validation sequencing variants, passed", dataframe=accept
            )
            logger.Messages.dataframe_size(
                label="...validation sequencing variants, rejected", dataframe=reject
            )
        else:
            logger.Messages.general(
                message="No input file for validation sequencing variants provided"
            )
            logger.Messages.general(message="...no input file provided to load")
            accept, reject = features.MAFValidation.import_feature(
                handle=path, config=config
...
class Process:
    ...
    def aneuploidy(df, dbs, cancer_type, config):
        if not df.empty:
            ...
            annotated = annotator.Annotator.annotate_almanac(
                df=df, dbs=dbs, ontology=cancer_type, config=config
            )
            logger.Messages.general(
                message="Annotation of whole-genome doubling complete",
                add_line_break=True,
            )
            ...
            logger.Messages.general(
                message="Evaluation of whole-genome doubling complete",
                add_line_break=True,
            )
        else:
            logger.Messages.general(
                message="No whole-genome doubling provided. Skipping annotation and evaluation."
            )
            evaluated = features.Features.create_empty_dataframe()
        logger.Messages.general(
            message="Annotation and evaluation of whole-genome doubling complete",
            add_line_break=True,
        )
        return evaluated
    ...
    def cosmic_mutational_signatures(df, dbs, cancer_type, config):
        if not df.empty:
            ...
            logger.Messages.general(
                message="Evaluating annotated COSMIC mutational signatures"
            )
            evaluated = evaluator.Evaluator.evaluate_almanac(annotated)
            logger.Messages.general(
                message="Evaluation of COSMIC mutational signatures complete",
                add_line_break=True,
            )
        else:
            logger.Messages.general(
                message="No mutational signatures provided. Skipping annotation and evaluation."
            )
            evaluated = features.Features.create_empty_dataframe()
    ...
    def integrated(somatic, germline, dbs, config):
        logger.Messages.general(
            message="Annoating somatic variants for germline variants in the same gene"
        )
        annotated = annotator.OverlapSomaticGermline.append_germline_hits(
            somatic, germline
        )

        logger.Messages.general(
            message="Summarizing somatic, germline, copy number, and fusion variants per gene"
        )
        integrated = evaluator.Integrative.evaluate(somatic, germline, dbs, config)

        logger.Messages.general(
            message="Investigating somatic and germline overlap complete",
            add_line_break=True,
        )
    ...
    def germline_variants(df, dbs, ontology, config):
        if not df.empty:
            ...
            annotated = annotator.Annotator.annotate_germline(
                df=df, dbs=dbs, ontology=ontology, config=config
            )
            logger.Messages.general(
                message="Annotation of germline variants complete", add_line_break=True
            )
            ...
            logger.Messages.general(
                message="Evaluation of germline variants complete", add_line_break=True
            )
        else:
            logger.Messages.general(
                message="No germline variants detected. Skipping annotation and evaluation."
            )
            evaluated = features.Features.create_empty_dataframe()
        logger.Messages.general(
            message="Annotation and evaluation of germline variants complete",
            add_line_break=True,
        )
        return evaluated

    @staticmethod
    def microsatellite_status(
        somatic_variants, germline_variants, status, dbs, cancer_type, config
    ):
        ...
        variants = evaluator.Microsatellite.evaluate_variants(
            somatic=somatic_variants, germline=germline_variants
        )
        if not variants.empty:
            ...
            logger.Messages.general(
                message="...no variants related to microsatellite instability observed."
            )
        message = "Evaluation of microsatellite instability variants complete"
        logger.Messages.general(message=message, add_line_break=True)

        logger.Messages.general(
            message="Annotating and evaluating microsatellite status"
        )
        if not status.empty:
            logger.Messages.general(message="Annotating microsatellite status")
            annotated = annotator.Annotator.annotate_almanac(
                df=status, dbs=dbs, ontology=cancer_type, config=config
            )
            logger.Messages.general(
                message="Annotation of microsatellite status complete",
                add_line_break=True,
            )
            ...
            evaluated = evaluator.Evaluator.evaluate_somatic(annotated)
            logger.Messages.general(
                message="Evaluation of microsatellite status complete",
                add_line_break=True,
            )
        else:
            logger.Messages.general(
                message="No microsatellite status provided. Skipping annotation and evaluation."
            )
            evaluated = features.Features.create_empty_dataframe()
    ...
    def somatic_variants(df, dbs, ontology, config, path_validation):
        if not df.empty:
            ...
            logger.Messages.general(
                message="Annotation of somatic variants complete", add_line_break=True
            )

            logger.Messages.general(message="Evaluating annotated somatic variants")
            evaluated = evaluator.Evaluator.evaluate_somatic(annotated)
            logger.Messages.general(
                message="Evaluation of somatic variants complete", add_line_break=True
            )
            ...
            if not validation_accept.empty:
                logger.Messages.general(
                    message="Annotating somatic variants with validation sequencing variants"
                )
                ...
                logger.Messages.general(
                    message="No passed validation sequencing variants to annotate with"
                )
        else:
            logger.Messages.general(
                message="No somatic variants detected. Skipping annotation and evaluation."
            )
            evaluated = features.Features.create_empty_dataframe()
        logger.Messages.general(
            message="Annotation and evaluation of somatic variants complete",
            add_line_break=True,
        )
        return evaluated
    ...
    def tumor_mutational_burden(df, dbs, cancer_type, config):
        if not df.empty:
            ...
            logger.Messages.general(
                message="Annotation of tumor mutational burden complete",
                add_line_break=True,
            )
            ...
            evaluated = evaluator.Evaluator.evaluate_somatic(annotated)
            logger.Messages.general(
                message="Evaluation of tumor mutational burden complete",
                add_line_break=True,
            )
        else:
            logger.Messages.general(
                message="No tumor mutational burden provided. Skipping annotation and evaluation."
            )
            evaluated = features.Features.create_empty_dataframe()
        logger.Messages.general(
            message="Annotation and evaluation of tumor mutational burden complete",
            add_line_break=True,
        )
        return evaluated
...
def format_metadata_dictionary(dictionary):
    ...
    if isinstance(dictionary[ms_status], str):
        dictionary[ms_status] = (
            "unk"
            if dictionary[ms_status] not in ["msih", "msil", "mss"]
            else dictionary[ms_status]
        )
    else:
...
def start_logging(patient, inputs, output_folder, config, dbs, dbs_preclinical):
    ...
    if dbs_preclinical:
        logger.Messages.inputs(
            label="Datasources for preclinical modules", dictionary=dbs_preclinical
        )
    else:
        logger.Messages.general(
            message="Datasources for preclinical modules not detected"
        )
...
def process_preclinical_efficacy(
    dbs, dataframe, folder, label, config, plot: bool = False
):
    efficacy_dictionary = investigator.SensitivityDictionary.create(
        dbs, dataframe, config
    )
    if plot:
        plot_preclinical_efficacy(efficacy_dictionary, folder, label)
    efficacy_summary = investigator.SummaryDataFrame.create(
        efficacy_dictionary, dataframe, label
    )
...
def main(patient, inputs, output_folder, config, dbs, dbs_preclinical=None):
    ...
    mapped_ontology = ontologymapper.OntologyMapper.map(
        dbs, metadata_dictionary[tumor_type]
    )
    ...
    df_snv, df_snv_reject = Load.somatic_variants(
        path=inputs[snv_handle], config=config
    )
    df_indel, df_indel_reject = Load.somatic_indels(
        path=inputs[indel_handle], config=config
    )
    df_cnv, df_cnv_reject = Load.copy_number(
        path_called=inputs[called_cn_handle], path_seg=inputs[cnv_handle], config=config
    )
    df_fusion, df_fusion_reject = Load.fusions(
        path=inputs[fusion_handle], config=config
    )
    ...
    germline_variants, germline_reject = Load.germline(
        path=inputs[germline_handle], config=config
    )
    ...
    evaluated_somatic, integrated = Process.integrated(
        somatic=evaluated_somatic, germline=evaluated_germline, dbs=dbs, config=config
    )
    ...
    cosmic_mutational_signatures = Load.cosmic_mutational_signatures(
        path=inputs[mutational_signatures_path], config=config
    )
    ...
    tumor_whole_genome_doubling = Load.aneuploidy(
        boolean=metadata_dictionary[wgd], config=config
    )
    tumor_microsatellite_status = Load.microsatellite_stability(
        boolean=metadata_dictionary[ms_status], config=config
    )
    metadata_dictionary[ms_status] = features.MicrosatelliteReader.map_status(
        metadata_dictionary[ms_status]
    )
    logger.Messages.general(
        message="Importing of second-order genomic features complete",
        add_line_break=True,
    )

    logger.Messages.header(
        label="Annotating and evaluating second-order genomic features"
    )
    ...
    evaluated_microsatellite_status, evaluated_microsatellite_status_variants = (
        Process.microsatellite_status(
    ...
    logger.Messages.general(
        message="Summary of clinically relevant biomarkers complete",
        add_line_break=True,
    )

    logger.Messages.header(
        label="Summarizing highlighted therapeutic strategies from clinically relevant hits"
    )
    strategies = evaluator.Strategies.report_therapy_strategies(actionable)
    logger.Messages.general(
        message="Summary of highlighted therapeutic strategies complete",
        add_line_break=True,
    )
    ...
    preclinical_efficacy_on = function_toggle.getboolean(
    ...
    model_similarity_on = (
        function_toggle.getboolean("calculate_model_similarity")
        and not inputs[disable_matchmaking]
    )
    ...
    if dbs_preclinical is not None:
        if preclinical_efficacy_on or model_similarity_on:
            ...
            if preclinical_efficacy_on:
                ...
                actionable = annotator.PreclinicalEfficacy.annotate(
                    ...
                    efficacy_summary,
                    efficacy_dictionary,
                    append_lookup=function_toggle.getboolean(
                        "include_preclinical_efficacy_in_actionability_report"
                    ),
                )
                logger.Messages.general(message=f"...annotation complete.")
                logger.Messages.general(
            ...
            if model_similarity_on and not cgc_empty:
                ...
                logger.Messages.general(
                    message="...genomic similarity to cancer cell lines complete. Summarizing..."
                )
                similarity_summary = matchmaker.Report.create_report_dictionary(
                    similarity_results, cell_lines_dictionary
                )
                logger.Messages.general(
                    message="...genomic similarity to cancer cell lines summarized"
                )
                logger.Messages.general(
                    message="Genomic similarity to cancer cell lines complete",
                    add_line_break=True,
                )
            elif cgc_empty:
                logger.Messages.general(
        ...
        logger.Messages.general(
            message="No preclinical databases provided, skipping", add_line_break=True
        )

    logger.Messages.header(label="Writing outputs")
    output_actionable = writer.Actionable.write(actionable, string_id, output_folder)
    output_g_acmg = writer.GermlineACMG.write(
        evaluated_germline, string_id, output_folder
    )
    output_g_cancer = writer.GermlineCancer.write(
        evaluated_germline, string_id, output_folder
    )
    output_g_hereditary = writer.GermlineHereditary.write(
        evaluated_germline, string_id, output_folder
    )
    output_integrated = writer.Integrated.write(integrated, string_id, output_folder)
    output_metadata = writer.Metadata.write(
        metadata_dictionary, string_id, output_folder
    )
    output_msi = writer.MSI.write(
        evaluated_microsatellite_status_variants, string_id, output_folder
    )
    output_tmb = writer.MutationalBurden.write(
        evaluated_tumor_mutational_burden, string_id, output_folder
    )
    output_somatic_scored = writer.SomaticScored.write(
        evaluated_somatic, string_id, output_folder
    )
    output_somatic_filtered = writer.SomaticFiltered.write(
        somatic_filtered, string_id, output_folder
    )
    output_strategies = writer.Strategies.write(strategies, string_id, output_folder)
    output_preclinical_efficacy = writer.PreclinicalEfficacy.write(
        efficacy_summary, string_id, output_folder
    )
    output_preclinical_matchmaking = writer.PreclinicalMatchmaking.write(
        similarity_results, string_id, output_folder
    )
    ...
    if function_toggle.getboolean("generate_figures"):
        ...
        logger.Messages.general(
            message="Figure generation completed", add_line_break=True
        )
    else:
        logger.Messages.general(
            message="Generating figures disabled, skipping", add_line_break=True
        )
    ...
    if function_toggle.getboolean("generate_actionability_report"):
        ...
        report_dictionary = reporter.Reporter.generate_dictionary(
            evaluated_somatic, metadata_dictionary
        )

        include_similarity = function_toggle.getboolean(
        ...
        logger.Messages.general(
            message="Report generation completed", add_line_break=True
        )
    else:
        logger.Messages.general(
            message="Generating report disabled, skipping", add_line_break=True
        )
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--patient_id", help="patient id label", required=True)
    arg_parser.add_argument("--description", default="", help="description of patient")
    arg_parser.add_argument(
        "--tumor_type",
        default="Unknown",
        help="reported tumor type, will attempt to map to both OncoTree codes and terms.",
    )
    arg_parser.add_argument("--stage", default="Unknown", help="disease stage")
    arg_parser.add_argument("--snv_handle", default="", help="handle for SNV MAF")
    arg_parser.add_argument("--indel_handle", default="", help="handle for InDel MAF")
    arg_parser.add_argument(
    ...
    arg_parser.add_argument(
        "--cnv_handle", default="", help="handle for annotated seg file"
    )
    arg_parser.add_argument(
        "--fusion_handle",
        default="",
    ...
    arg_parser.add_argument(
        "--germline_handle", default="", help="handle for Germline MAF"
    )
    arg_parser.add_argument(
        "--validation_handle",
        default="",
    ...
    arg_parser.add_argument("--purity", default="Unknown", help="Tumor purity")
    arg_parser.add_argument("--ploidy", default="Unknown", help="Tumor ploidy")
    arg_parser.add_argument(
        "--wgd",
        action="store_true",
    ...
    arg_parser.add_argument(
        "--output_directory", default=None, help="Output directory for generated files"
    )
    arg_parser.add_argument(
        "--config",
        "-c",
    ...
    arg_parser.add_argument(
        "--dbs", required=True, help="ini file that contains database paths"
    )
    arg_parser.add_argument(
        "--preclinical-dbs",
        required=False,
