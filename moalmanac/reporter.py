...
class Reporter:
    ...
    def drop_double_fusion(cls, dataframe, biomarker_type_string):
        ...
        idx_rearrangement = dataframe[
            dataframe[feature_type].eq(biomarker_type_string)
        ].index
        idx_rearrangement_keep = (
            dataframe.loc[idx_rearrangement, :]
    ...
    def format_alterations(cls, dataframe, config):
        ...
        columns = [
            lookup["sensitive_implication"],
            lookup["resistance_implication"],
        ...
        columns = [
            lookup["sensitivity_matches"],
            lookup["resistance_matches"],
            lookup["prognostic_matches"],
        ]
        for column in columns:
    ...
    def format_clinical_columns(series, convert_to_float=False):
        series = series.where(series.notna(), None).where(~series.eq(""), None)
        if convert_to_float:
    ...
    def generate_actionability_report(
        ...
        app.config["FREEZER_DESTINATION"] = (
            f"{os.getcwd()}" if output_directory == ("" or None) else output_directory
        )
        app.config["FREEZER_REMOVE_EXTRA_FILES"] = (
            False  # DO NOT REMOVE THIS, FLASK FROZEN WILL DELETE FILES IF TRUE
        )
    ...
    def generate_version_dictionary(cls, config):
        ...
        return {"software": software_version, "database": database_version}
