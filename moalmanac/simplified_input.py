...
def subset_by_feature_type(dataframe):
    ...
    somatic = dataframe[dataframe[feature_type].isin(somatic_types)].reset_index(
        drop=True
    )
    germline = dataframe[dataframe[feature_type].isin(germline_types)].reset_index(
        drop=True
    )
...
def main(patient, input_file, output_folder, config, dbs, dbs_preclinical=None):
    ...
    annotated_alterations = annotator.Annotator.annotate_simple(
        alterations, dbs, patient[code], config=config
    )
    evaluated_alterations = evaluator.Evaluator.evaluate_somatic(annotated_alterations)

    evaluated_somatic, evaluated_germline = subset_by_feature_type(
        evaluated_alterations
    )
    evaluated_somatic = annotator.OverlapSomaticGermline.append_germline_hits(
        evaluated_somatic, evaluated_germline
    )
    integrated = evaluator.Integrative.evaluate(
        evaluated_somatic, evaluated_germline, dbs, feature_types
    )
    ...
    patient_ms_status = features.MicrosatelliteReader.summarize(
        patient[ms_status], config=config
    )
    ...
    annotated_burden = annotator.Annotator.annotate_almanac(
        somatic_burden, dbs, patient[code], config=config
    )
    annotated_wgd = annotator.Annotator.annotate_almanac(
        patient_wgd, dbs, patient[code], config=config
    )
    annotated_ms_status = annotator.Annotator.annotate_almanac(
        patient_ms_status, dbs, patient[code], config=config
    )
    ...
    evaluated_ms_variants = evaluator.Microsatellite.evaluate_variants(
        evaluated_somatic, evaluated_germline
    )
    evaluated_ms_status = evaluator.Microsatellite.evaluate_status(
        annotated_ms_status, evaluated_ms_variants
    )
    ...
    preclinical_efficacy_on = TOGGLE_FEATURES.getboolean(
    ...
    if preclinical_efficacy_on or model_similarity_on:
        ...
        if preclinical_efficacy_on:
            ...
            actionable = annotator.PreclinicalEfficacy.annotate(
                ...
                efficacy_summary,
                efficacy_dictionary,
                append_lookup=TOGGLE_FEATURES.getboolean(
    ...
    writer.SomaticFiltered.write(
        features.Features.create_empty_dataframe(), string_id, output_folder
    )
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(
        prog="Molecular Oncology Almanac using simplified input",
    ...
    arg_parser.add_argument(
        "--tumor_type", default="Unknown", help="reported tumor type"
    )
    arg_parser.add_argument(
        "--input", help="Tab delimited file of observed alterations"
    )
    arg_parser.add_argument(
        "--ms_status",
        default="unk",
    ...
    main(
        ...
        dbs_preclinical=(
            preclinical_db_paths["preclinical"] if preclinical_db_paths else None
        ),
    )
