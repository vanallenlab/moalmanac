...
class Features:
    ...
    def concat_list_of_dataframes(cls, list_of_dataframes, ignore_index=True):
        ...
        non_empty_dataframes = [
            dataframe for dataframe in list_of_dataframes if not dataframe.empty
        ]
        # Remove dataframes with only null values
        dataframes = [
            dataframe
            for dataframe in non_empty_dataframes
            if not dataframe.isnull().all().all()
        ]
...
class CopyNumberTotal(CopyNumber):
    ...
    def filter_by_threshold(
        ...
        idx_amp = df[
            df[Features.segment_mean].astype(float) >= float(threshold_amp)
        ].index
        idx_del = df[
            df[Features.segment_mean].astype(float) <= float(threshold_del)
        ].index
...
class CoverageMetrics:
    ...
    def calculate_tumor_f(series_alt_count, series_total_coverage):
        ...
        return (
            series_alt_count.divide(series_total_coverage)
            .dropna()
            .astype(float)
            .round(4)
        )
    ...
    def convert_to_pandas_int64(cls, series):
        return series.apply(cls.safe_cast).astype("Int64")
...
class Fusion:
    ...
    def split_breakpoint(series_breakpoint):
        return (
            series_breakpoint.str.split(":", expand=True)
...
class MicrosatelliteReader:
    ...
    status_map = {"msih": msih, "msil": msil, "mss": mss, "unk": unk}
