...
class Annotator:
    ...
    def fill_na(cls, dataframe, column, fill_value, fill_data_type):
        ...
        return (
            dataframe.loc[dataframe.index, column]
            .astype(fill_data_type)
            .fillna(fill_value)
        )
    ...
    def get_idx_without_na(cls, dataframe, columns, datasource_label):
        ...
        message = (
            f"...{removed_rows} of {dataframe.shape[0]} rows have null values in "
...
class Almanac:
    ...
    sensitive_feature_display = COLNAMES[datasources_section][
        "sensitive_feature_display"
    ]
    ...
    resistance_feature_display = COLNAMES[datasources_section][
        "resistance_feature_display"
    ]
    ...
    prognostic_feature_display = COLNAMES[datasources_section][
        "prognostic_feature_display"
    ]
    ...
    def annotate(cls, df, dbs, ontology, config):
        ...
        annotation_function_dict = {
            ...
            config["feature_types"][
                "microsatellite"
            ]: cls.annotate_microsatellite_stability,
    ...
    def annotate_burden(cls, sliced_series, ontology, table):
        ...
        for assertion_type in cls.assertion_types_dict.keys():
            ...
            columns[cls.classification] = cls.assertion_types_dict[assertion_type][
                cls.feature
            ]
    ...
    def annotate_copy_number(cls, sliced_series, ontology, table):
        ...
        for assertion_type in cls.assertion_types_dict.keys():
            ...
            columns[cls.direction] = cls.assertion_types_dict[assertion_type][
                cls.alt_type
            ]
            ...
            if (query_to_alt_type & query).any():
                ...
                message = (
                    "......was able to match on gene, biomarker type, and direction"
                )
                logger.Messages.general(message=message)
                ...
                message = (
                    "......was unable to match on gene, biomarker type, and direction"
                )
                logger.Messages.general(message=message)
    ...
    def annotate_fusion(cls, sliced_series, ontology, table):
        ...
        for assertion_type in cls.assertion_types_dict.keys():
            message = (
                f"...annotating {feature} {alt_type} {alt} for {assertion_type}..."
            )
            logger.Messages.general(message=message)
            ...
            columns[cls.rearrangement_type] = cls.assertion_types_dict[assertion_type][
                cls.alt_type
            ]
            alt_column = cls.assertion_types_dict[assertion_type][cls.alt]

            fusion_matches = []
            for first_gene, second_gene, feature_col in [
                (gene1, gene2, cls.gene1),
                (gene2, gene1, cls.gene2),
            ]:
                columns[feature_col] = cls.assertion_types_dict[assertion_type][
                    cls.feature
                ]
                ...
                query_to_alt_reverse = (
                    query_first_gene_reverse
                    & query_second_gene_reverse
                    & query_alt_type
                )
                ...
                if (query_to_alt & query).any():
                    ...
                    results_same_ontology = (
                        query_same_ontology & query_to_alt_reverse & query
                    )
                    results_diff_ontology = (
                        query_diff_ontology & query_to_alt_reverse & query
                    )
                    match_bin = 4
                    ...
                    results_same_ontology = (
                        query_same_ontology & query_to_alt_type & query
                    )
                    results_diff_ontology = (
                        query_diff_ontology & query_to_alt_type & query
                    )
                    match_bin = 3
                    ...
                    results_same_ontology = (
                        query_same_ontology & query_to_alt_type_reverse & query
                    )
                    results_diff_ontology = (
                        query_diff_ontology & query_to_alt_type_reverse & query
                    )
                    match_bin = 3
                    ...
                    results_same_ontology = (
                        query_same_ontology & query_first_gene & query
                    )
                    results_diff_ontology = (
                        query_diff_ontology & query_first_gene & query
                    )
                    match_bin = 2
                    ...
                    results_same_ontology = (
                        query_same_ontology & query_second_gene & query
                    )
                    results_diff_ontology = (
                        query_diff_ontology & query_second_gene & query
                    )
                    match_bin = 2
                    ...
                    message = (
                        "......was able to match on second gene and biomarker type"
                    )
                    logger.Messages.general(message=message)
    ...
    def annotate_signatures(cls, sliced_series, ontology, table):
        ...
        for assertion_type in cls.assertion_types_dict.keys():
            ...
            columns[cls.cosmic_signature_number] = cls.assertion_types_dict[
                assertion_type
            ][cls.feature]
    ...
    def annotate_variants(cls, sliced_series, ontology, table):
        ...
        for assertion_type in cls.assertion_types_dict.keys():
            ...
            columns[cls.gene] = cls.assertion_types_dict[assertion_type][cls.feature]
            columns[cls.variant_annotation] = cls.assertion_types_dict[assertion_type][
                cls.alt_type
            ]
            columns[cls.protein_change] = cls.assertion_types_dict[assertion_type][
                cls.alt
            ]
    ...
    def subset_list_of_dictionaries(list_of_dicts, key, operator, subset_value):
        return [
            ...
            if operator(dictionary[key], subset_value)
        ]
...
class GSEACancerPathways:
    ...
    def annotate(cls, df, dbs):
        ...
        return Annotator.annotate(
            df,
            dbs,
            datasources.GSEACancerPathways,
            cls.bin_name,
...
class OverlapSomaticGermline:
    ...
    def merge_germline_hits(cls, somatic, germline_counts):
        return pd.merge(
            somatic.drop([cls.number_germline_mutations], axis=1),
            germline_counts,
            on=[cls.gene],
            how="left",
        )
...
class PreclinicalEfficacy:
    ...
    def annotate(cls, actionable, efficacy, dictionary, append_lookup=True):
        ...
        series_features = series_all_features[
            series_all_features.isin(efficacy[cls.feature_display])
        ]
...
class PreclinicalMatchmaking:
    ...
    def annotate_fusions(cls, df, dbs, biomarker_type_string):
        ...
        db_gene1 = (
            db.copy(deep=True)
            .rename(columns={cls.gene1: cls.feature, cls.gene2: cls.partner})
            .fillna("")
        )
        db_gene2 = (
            db.copy(deep=True)
            .rename(columns={cls.gene1: cls.partner, cls.gene2: cls.feature})
            .fillna("")
        )
    ...
    def annotate_match_3(cls, df, db, feature=feature, alteration_type=alteration_type):
        ...
        df = df.merge(
            db.loc[(~db[feature].eq("") & ~db[alteration_type].eq("")), :]
            .loc[:, match_columns + [cls.evidence_map_str, cls.merged]]
            .sort_values(cls.evidence_map_str, ascending=False)
            .drop_duplicates(match_columns, keep="first"),
            on=match_columns,
