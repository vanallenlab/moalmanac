...
class UnitTestFeatures(unittest.TestCase):
    ...
    def test_create_empty_dataframe(self):
        self.assertEqual(
            True, features.Features.create_empty_dataframe().dropna().empty
        )
    ...
    def test_drop_duplicate_genes(self):
        ...
        dataframe = pd.DataFrame(
            {feature: ["ABL1", "BRAF", "BRAF", "TP53"], "sort": [0, 0, 1, 1]}
        )
        self.assertEqual(
            [2, 3, 0],
            features.Features.drop_duplicate_genes(dataframe, "sort").tolist(),
        )
    ...
    def test_import_if_path_exists(self):
        ...
        imported_dataframe = features.Features.import_if_path_exists(
            handle, "\t", column_map
        )
        ...
        self.assertEqual(
            sorted(empty_dataframe.columns.tolist()),
            sorted(imported_dataframe.columns.tolist()),
        )
    ...
    def test_preallocate_missing_columns(self):
        ...
        self.assertEqual(
            columns,
            features.Features.preallocate_missing_columns(
                empty_dataframe
            ).columns.tolist(),
        )
        self.assertEqual(
...
class UnitTestCopyNumber(unittest.TestCase):
    ...
    def test_format_cn_gene(self):
        ...
        self.assertEqual(
            ["A", "A", "B"], features.CopyNumber.format_cn_gene(series).tolist()
        )
...
class UnitTestCopyNumberCalled(unittest.TestCase):
    ...
    def test_filter_calls(self):
        ...
        idx = features.CopyNumberCalled.filter_calls(
            series=tmp, amp_string=amp_string, del_string=del_string
        )
...
class UnitTestCopyNumberTotal(unittest.TestCase):
    ...
    def test_create_column_map(self):
        ...
        self.assertEqual(
            column_map[column_names["mean"]], features.Features.segment_mean
        )
    ...
    def test_drop_duplicate_genes(self):
        ...
        df = pd.DataFrame(
            {features.Features.feature: genes, features.Features.segment_mean: segments}
        )
        for idx in expected:
            self.assertEqual(
                True, idx in features.CopyNumberTotal.drop_duplicate_genes(df)
            )
    ...
    def test_get_unique_segments(self):
        df = pd.DataFrame(
            {
                features.Features.chr: [1, 1, 2, 2, 3, 4],
        ...
        expected = [0, 2, 3, 4, 5]
        self.assertEqual(
            expected, features.CopyNumberTotal.get_unique_segments(df).tolist()
        )
...
class UnitTestCosmicSignatures(unittest.TestCase):
    ...
    def test_subset_significant_signatures(self):
        ...
        series = pd.Series(contributions)
        idx = features.CosmicSignatures.index_for_minimum_contribution(
            series, minimum_value=0.20
        )
        result = series[idx]
...
class UnitTestCoverageMetrics(unittest.TestCase):
    ...
    def test_calculate_coverage(self):
        ...
        result = CoverageMetrics.calculate_coverage(
            series_alt_count=series_alt_counts, series_ref_count=series_ref_counts
        )
    ...
    def test_calculate_tumor_f(self):
        ...
        result = CoverageMetrics.calculate_tumor_f(
    ...
    def test_get_minimum_values_dataframe(self):
        ...
        result = features.CoverageMetrics.get_minimum_values_dataframe(
            dataframe, axis=1
        )
        self.assertEqual(first=3, second=result.loc[0])
        self.assertEqual(first=4, second=result.loc[4])
...
class UnitTestFusion(unittest.TestCase):
    def test_create_column_map(self):
        config = Ini.read(
            path="config.ini", extended_interpolation=False, convert_to_dictionary=False
        )
...
class UnitTestMAF(unittest.TestCase):
    ...
    def test_format_maf(self):
        ...
        self.assertEqual(
            "Somatic Variant", result[features.MAF.feature_type].unique().tolist()[0]
        )
        self.assertEqual(
            1, result[features.MAF.feature_type].drop_duplicates().shape[0]
        )
    ...
    def test_return_variants_coding(self):
        initial = pd.DataFrame(
            {features.MAF.alt_type: ["Missense", "Silent", "A", 1, pd.NA]}
        )
