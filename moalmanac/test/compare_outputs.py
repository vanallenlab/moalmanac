...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="compare outputs",
        description="Compare changed columns relative to unchanged columns",
    )
    ...
    parser.add_argument(
        ...
        "-u",
        action="append",
        help="columns that did not change",
        required=True,
    )
    parser.add_argument(
        "--column_changed", "-c", help="column that did change", required=True
    )
    ...
    idx_difference = merged[f"{args.column_changed}_x"].ne(
        merged[f"{args.column_changed}_y"]
    )
    result = merged.loc[idx_difference, :].sort_values(
        by=[f"{args.column_changed}_x", f"{args.column_changed}_y"],
        axis=0,
        ascending=True,
    )
