...
class UnitTestSensitivityDictionary(unittest.TestCase):
    def test_calculate_series_exp(self):
        ...
        expected = pd.Series(
            [1.000, 2.718, 7.389, 20.086, 148.413, 7.389], name="value"
        )
    ...
    def test_calculate_wilcoxon_rank_sum(self):
        ...
        statistic, pvalue = SensitivityDictionary.calculate_wilcoxon_rank_sum(
            series1, series2
        )
    ...
    def test_calculate_mann_whitney_u(self):
        ...
        series2 = pd.Series([1, 2, 3, 4, 5, 6], name="value")
        series3 = pd.Series([], dtype=float, name="value")
        statistic, pvalue = SensitivityDictionary.calculate_mann_whitney_u(
            series1, series2
        )
    ...
    def test_create(self):
        ...
        config = Ini.read(
            "config.ini", extended_interpolation=False, convert_to_dictionary=False
        )
        result = SensitivityDictionary.create(dbs, actionable, config)
        self.assertAlmostEqual(
            result[0]["Dabrafenib"]["BRAF"]["comparison"]["pvalue_mww"],
            expected_dabrafenib,
            places=10,
        )
        self.assertAlmostEqual(
            result[0]["Trametinib"]["BRAF"]["comparison"]["pvalue_mww"],
            expected_trametinib,
            places=10,
        )
    ...
    def test_create_drug_dict(self):
        ...
        test_dataframe = pd.DataFrame(
            {SensitivityDictionary.model_id: all_samples, "feature": [0, 1, 2, pd.NA]}
        )
        groups = [
    ...
    def test_generate_feature_strings(self):
        series = pd.Series(
            {"feature_type": "Copy Number", "feature": "CDKN2A", "alt_type": "Deletion"}
        )
        labels = [
            ["feature", "feature_type", "alt_type"],
            ["feature", "feature_type"],
            ["feature"],
        ]
        result = SensitivityDictionary.generate_feature_strings(labels, series)
        self.assertEqual(
            result, ["CDKN2A Copy Number Deletion", "CDKN2A Copy Number", "CDKN2A"]
        )
    ...
    def test_select_split_function(self):
        ...
        self.assertEqual(
            SensitivityDictionary.select_split_function(
                var_string, var_string, cn_string, fusion_string
            ),
            var_function,
        )
        self.assertEqual(
            SensitivityDictionary.select_split_function(
                cn_string, var_string, cn_string, fusion_string
            ),
            cn_function,
        )
        self.assertEqual(
            SensitivityDictionary.select_split_function(
                fusion_string, var_string, cn_string, fusion_string
            ),
            fusion_function,
        )
    ...
    def test_split_samples_by_wt_mut(self):
        ...
        data = pd.DataFrame(
            {
                SensitivityDictionary.feature_type: [
                    "Somatic Variant",
                    "Copy Number",
                    "Rearrangement",
                    "Aneuploidy",
                ...
                SensitivityDictionary.alteration_type: [
                    "Missense",
                    "Deletion",
                    "Fusion",
                    "",
        ...
        db_variants = pd.DataFrame(
            {
                ...
                SensitivityDictionary.alteration: [
                    "p.V600E",
                    "p.V600K",
                    "p.K500A",
                    "p.V600*",
        ...
        results_variants = SensitivityDictionary.split_samples_by_wt_mut(
            data.loc[0, :], dbs, samples, config
        )
        results_cnas = SensitivityDictionary.split_samples_by_wt_mut(
            data.loc[1, :], dbs, samples, config
        )
        results_fusions = SensitivityDictionary.split_samples_by_wt_mut(
            data.loc[2, :], dbs, samples, config
        )
        ...
        self.assertEqual(
            results_variants["BRAF Somatic Variant"]["samples"][1], ["A", "B", "C", "D"]
        )
        self.assertEqual(
            results_variants["BRAF Somatic Variant Missense"]["samples"][0], ["D", "E"]
        )
        self.assertEqual(
            results_variants["BRAF Somatic Variant Missense"]["samples"][1],
            ["A", "B", "C"],
        )
        self.assertEqual(
            results_variants["BRAF Somatic Variant Missense p.V600E"]["samples"][0],
            ["B", "C", "D", "E"],
        )
        self.assertEqual(
            results_variants["BRAF Somatic Variant Missense p.V600E"]["samples"][1],
            ["A"],
        )
        self.assertEqual(results_cnas["CDKN2A"]["samples"][0], ["A", "C", "D", "E"])
        self.assertEqual(results_cnas["CDKN2A"]["samples"][1], ["B"])
        self.assertEqual(
            results_cnas["CDKN2A Copy Number"]["samples"][0], ["A", "D", "E"]
        )
        self.assertEqual(results_cnas["CDKN2A Copy Number"]["samples"][1], ["B", "C"])
        self.assertEqual(
            results_cnas["CDKN2A Copy Number Deletion"]["samples"][0],
            ["A", "C", "D", "E"],
        )
        self.assertEqual(
            results_cnas["CDKN2A Copy Number Deletion"]["samples"][1], ["B"]
        )
        ...
        self.assertEqual(
            results_fusions["TMPRSS2 Fusions"]["samples"][0], ["A", "B", "E"]
        )
        self.assertEqual(results_fusions["TMPRSS2 Fusions"]["samples"][1], ["C", "D"])
        self.assertEqual(
            results_fusions["ERG Fusions"]["samples"][0], ["A", "B", "D", "E"]
        )
        self.assertEqual(results_fusions["ERG Fusions"]["samples"][1], ["C"])
        self.assertEqual(
            results_fusions["TMPRSS2--ERG"]["samples"][0], ["A", "B", "D", "E"]
        )
    ...
    def test_split_samples_for_fusions(self):
        ...
        dbs = {gene: db_genes, fusions: db_fusions}
    ...
    def test_split_samples_for_variants(self):
        ...
        dbs = {gene: db_genes, variants: db_variants}
    ...
    def test_retrieve_mut_samples(self):
        ...
        self.assertEqual(
            SensitivityDictionary.retrieve_mut_samples(db, condition),
            ["A", "B", "C", "D"],
        )
    ...
    def test_retrieve_wt_samples(self):
        samples = ["A", "B", "C", "D", "E"]
        self.assertEqual(
            SensitivityDictionary.retrieve_wt_samples(samples, ["A", "B"]),
            ["C", "D", "E"],
        )
        self.assertEqual(
            SensitivityDictionary.retrieve_wt_samples(samples, []), samples
        )
    ...
    def test_round_scientific_notation(self):
        self.assertEqual(
            SensitivityDictionary.round_scientific_notation(0.0015, 3), 0.002
        )
        self.assertEqual(
            SensitivityDictionary.round_scientific_notation(0.00000123, 2), "1.23e-06"
        )
