...
class UnitTestWriter(unittest.TestCase):
    ...
    def test_sort_columns(self):
        ...
        self.assertEqual(
            [0, 1, 2], list(Writer.sort_columns(df, ["A", "B"], [True, True]).index)
        )
        self.assertEqual(
            [2, 1, 0], list(Writer.sort_columns(df, ["A", "B"], [False, True]).index)
        )
        self.assertEqual(
            [1, 0, 2], list(Writer.sort_columns(df, ["B", "A"], [True, False]).index)
        )
        self.assertEqual(
            [2, 0, 1], list(Writer.sort_columns(df, ["B", "A"], [False, True]).index)
        )
